...
function DockItem({
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className={`relative inline-flex items-center justify-center rounded-full bg-card border-border border-2 shadow-md transition-colors hover:bg-accent/50 ${className}`}
      tabIndex={0}
      role="button"
      aria-haspopup="true"
    >
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className={`${className} absolute -top-8 left-1/2 w-fit whitespace-pre rounded-md border border-border bg-card px-2 py-1 text-xs text-foreground shadow-lg`}
          role="tooltip"
          style={{ x: '-50%' }}
        >
          {children}
